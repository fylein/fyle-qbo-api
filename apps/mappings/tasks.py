...
def disable_category_for_items_mapping(workspace_id: int):
    ...
    qbo_credentials: QBOCredential = QBOCredential.get_active_qbo_credentials(
        workspace_id
    )
    ...
    category_ids_to_be_disabled = disable_or_enable_expense_attributes(
        "CATEGORY", "ACCOUNT", workspace_id, display_name="Item"
    )
    if category_ids_to_be_disabled:
        expense_attributes = ExpenseAttribute.objects.filter(
            id__in=category_ids_to_be_disabled
        )
        fyle_payload: List[Dict] = create_fyle_categories_payload(
            categories=[],
            workspace_id=workspace_id,
            updated_categories=expense_attributes,
        )
...
def disable_or_enable_expense_attributes(
    source_field: str,
    ...
    if expense_attributes_to_disable or expense_attributes_to_enable:
        ...
        if source_field not in ("CATEGORY", "PROJECT"):
            expense_attributes_ids = expense_attributes_ids + [
                expense_attribute.id
                for expense_attribute in expense_attributes_to_enable
            ]
...
def post_projects_in_batches(
    ...
    for offset in range(0, qbo_attributes_count, page_size):
        ...
        fyle_payload: List[Dict] = create_fyle_projects_payload(
            paginated_qbo_attributes, existing_project_names
        )
        if fyle_payload:
        ...
        Mapping.bulk_create_mappings(
            paginated_qbo_attributes, "PROJECT", destination_field, workspace_id
        )

    if destination_field == "CUSTOMER":
        project_ids_to_be_changed = disable_or_enable_expense_attributes(
...
def create_fyle_categories_payload(
    ...
    if updated_categories:
        ...
        existing_category_names_lower_case = [
            existing_category_name.lower()
            for existing_category_name in existing_category_names
        ]

        for category in categories:
            if (
                category.value.lower() not in existing_category_names_lower_case
                and category.value.lower() not in DEFAULT_FYLE_CATEGORIES
            ):
                payload.append(
                    {
                        "name": category.value,
...
def upload_categories_to_fyle(platform: PlatformConnector, workspace_id):
    ...
    if general_settings.import_items:
        ...
        qbo_items: List[DestinationAttribute] = DestinationAttribute.objects.filter(
            workspace_id=workspace_id, display_name="Item", attribute_type="ACCOUNT"
        ).all()

        if qbo_items:
            qbo_attributes = (
                qbo_attributes.union(qbo_items) if qbo_attributes else qbo_items
            )
...
def auto_create_category_mappings(workspace_id):
    ...
    category_mappings = Mapping.bulk_create_mappings(
        fyle_categories, "CATEGORY", "ACCOUNT", workspace_id
    )

    # disabling fields
    category_ids_to_be_changed = disable_or_enable_expense_attributes(
...
def check_exact_matches(
    ...
    source_attribute: ExpenseAttribute,
    ...
    if source_value.lower() in destination_id_value_map:
        if destination_type == "EMPLOYEE":
            destination["destination_employee_id"] = destination_id_value_map[
                source_value.lower()
            ]
        elif destination_type == "VENDOR":
            destination["destination_vendor_id"] = destination_id_value_map[
                source_value.lower()
            ]
        elif destination_type == "CREDIT_CARD_ACCOUNT":
            destination["destination_card_account_id"] = destination_id_value_map[
                source_value.lower()
            ]
...
def construct_mapping_payload(
    employee_source_attributes: List[ExpenseAttribute],
    employee_mapping_preference: str,
    destination_id_value_map: dict,
    destination_type: str,
    workspace_id: int,
    ...
    """
    existing_source_ids, existing_mappings = get_existing_source_and_mappings(
    ...
    existing_mappings_map = {
        mapping.source_employee_id: mapping.id for mapping in existing_mappings
    }
    ...
    for source_attribute in employee_source_attributes:
        ...
        if source_attribute.id not in existing_source_ids:
            destination = check_exact_matches(
...
def create_mappings_and_update_flag(
    mapping_creation_batch: List[EmployeeMapping],
    mapping_updation_batch: List[EmployeeMapping],
    update_key: str,
):
    """
    ...
    if mapping_creation_batch:
        created_mappings = EmployeeMapping.objects.bulk_create(
            mapping_creation_batch, batch_size=50
        )
    ...
    for mapping in mappings:
        expense_attributes_to_be_updated.append(
            ExpenseAttribute(id=mapping.source_employee.id, auto_mapped=True)
        )
...
def auto_map_employees(
    ...
    create_mappings_and_update_flag(
        mapping_creation_batch, mapping_updation_batch, update_key
    )
...
def auto_map_ccc_employees(default_ccc_account_id: str, workspace_id: int):
    ...
    existing_source_ids, existing_mappings = get_existing_source_and_mappings(
        "CREDIT_CARD_ACCOUNT", workspace_id
    )
    existing_mappings_map = {
        mapping.source_employee_id: mapping.id for mapping in existing_mappings
    }
    ...
    if mapping_updation_batch:
        EmployeeMapping.objects.bulk_update(
            mapping_updation_batch,
            fields=["destination_card_account_id"],
            batch_size=50,
        )
...
def upload_tax_groups_to_fyle(
    ...
    fyle_payload: List[Dict] = create_fyle_tax_group_payload(
        qbo_attributes, existing_tax_codes_name
    )
...
def create_fyle_cost_centers_payload(
    qbo_attributes: List[DestinationAttribute], existing_fyle_cost_centers: list
):
    """
            ...
            fyle_cost_centers_payload.append(
                {
                    ...
                    "is_enabled": True
                    if qbo_attribute.active is None
                    else qbo_attribute.active,
                    "description": "Cost Center - {0}, Id - {1}".format(
                        qbo_attribute.value, qbo_attribute.destination_id
                    ),
                }
            )
...
def create_fyle_tax_group_payload(
    qbo_attributes: List[DestinationAttribute], existing_fyle_tax_groups: list
):
    """
    Create Fyle Cost Centers Payload from QBO Objects
...
def post_cost_centers_in_batches(
    platform: PlatformConnector, workspace_id: int, qbo_attribute_type: str
):
    ...
    qbo_attributes_count = DestinationAttribute.objects.filter(
        attribute_type=qbo_attribute_type, workspace_id=workspace_id
    ).count()
    ...
    for offset in range(0, qbo_attributes_count, page_size):
        ...
        Mapping.bulk_create_mappings(
            paginated_qbo_attributes, "COST_CENTER", qbo_attribute_type, workspace_id
        )
...
def create_fyle_expense_custom_field_payload(
    qbo_attributes: List[DestinationAttribute],
    workspace_id: int,
    fyle_attribute: str,
    ...
    if fyle_attribute.lower() not in FYLE_EXPENSE_SYSTEM_FIELDS:
        ...
        if existing_attribute is not None:
            ...
            is_mandatory = (
                existing_attribute["is_mandatory"]
                if "is_mandatory" in existing_attribute
                else False
            )
            ...
            qbo_attributes = [
                qbo_attribute
                for qbo_attribute in qbo_attributes
                if qbo_attribute.updated_at > parser.parse(last_imported_at)
            ]

        [
            fyle_expense_custom_field_options.append(qbo_attribute.value)
            for qbo_attribute in qbo_attributes
        ]
        ...
        # source_field is the field that's save in mapping settings, this field user may or
        # may not fill in the custom field form
        # placeholder is the field that's saved in the detail column of destination attributes
        # fyle_attribute is what we're constructing when both of these field would not be available

        if not (source_placeholder or placeholder):
            # If source_placeholder and placeholder are both None, then we're creating adding a
            # self constructed placeholder
            new_placeholder = "Select {0}".format(fyle_attribute)
        elif not source_placeholder and placeholder:
            # If source_placeholder is None but placeholder is not, then we're choosing same place
            # holder as 1 in detail section
            new_placeholder = placeholder
        elif source_placeholder and not placeholder:
            # If source_placeholder is not None but placeholder is None, then we're choosing the
            # placeholder as filled by user in form
...
def auto_create_expense_fields_mappings(
    ...
    if fyle_attributes:
        Mapping.bulk_create_mappings(
            fyle_attributes, fyle_attribute_type, qbo_attribute_type, workspace_id
        )
...
def post_merchants(platform_connection: PlatformConnector, workspace_id: int):
    ...
    qbo_attributes = DestinationAttribute.objects.filter(
        attribute_type="VENDOR", active=True, workspace_id=workspace_id
    ).order_by("value", "id")
...
def auto_import_and_map_fyle_fields(workspace_id):
    ...
    if workspace_general_settings.import_vendors_as_merchants:
        chain.append(
            "apps.mappings.tasks.auto_create_vendors_as_merchants", workspace_id
        )

    if (
        workspace_general_settings.import_categories
        or workspace_general_settings.import_items
    ):
