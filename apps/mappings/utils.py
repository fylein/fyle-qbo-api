...
class MappingUtils:
    ...
    def create_or_update_general_mapping(self, general_mapping: Dict):
        ...
        if (
            ...
            and general_settings.reimbursable_expenses_object == "JOURNAL ENTRY"
        ) or (
            general_settings.corporate_credit_card_expenses_object == "BILL"
            or general_settings.reimbursable_expenses_object == "BILL"
        ):
            assert_valid(
        ...
        if general_settings.sync_fyle_to_qbo_payments:
            assert_valid(
                "bill_payment_account_name" in general_mapping
                and general_mapping["bill_payment_account_name"],
                "bill payment account name field is blank",
            )
            assert_valid(
                "bill_payment_account_id" in general_mapping
                and general_mapping["bill_payment_account_id"],
                "bill payment account id field is blank",
            )

        if general_settings.reimbursable_expenses_object == "EXPENSE":
            assert_valid(
                "qbo_expense_account_name" in general_mapping
                and general_mapping["qbo_expense_account_name"],
                "qbo expense account name field is blank",
            )
            assert_valid(
                "qbo_expense_account_id" in general_mapping
                and general_mapping["qbo_expense_account_id"],
        ...
        if (
            ...
            assert_valid(
                ...
                and general_mapping["default_debit_card_account_name"],
                "debit card account name field is blank",
            )
            assert_valid(
                "default_debit_card_account_id" in general_mapping
                and general_mapping["default_debit_card_account_id"],
                "debit card account id field is blank",
            )

        general_mapping_object, _ = GeneralMapping.objects.update_or_create(
            workspace_id=self.__workspace_id, defaults=general_mapping
        )
