...
class PreferencesView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                data={"message": "Quickbooks Online connection expired"},
                status=status.HTTP_400_BAD_REQUEST,
            )
            ...
            if qbo_credentials:
                ...
                qbo_credentials.save()
            return Response(
                data={"message": "Invalid token, try to refresh it"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class SyncQuickbooksDimensionView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if workspace.destination_synced_at is None or time_interval.days > 0:
            quickbooks_credentials = QBOCredential.get_active_qbo_credentials(
                kwargs["workspace_id"]
            )
            quickbooks_connector = QBOConnector(
                quickbooks_credentials, workspace_id=kwargs["workspace_id"]
            )
...
class RefreshQuickbooksDimensionView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        mapping_settings = MappingSetting.objects.filter(
            workspace_id=kwargs["workspace_id"], import_to_fyle=True
        )
        chain = Chain()
...
class QBOAttributesView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return DestinationAttribute.objects.filter(
            attribute_type__in=attribute_types, workspace_id=self.kwargs["workspace_id"]
