...
class Cheque(models.Model):
    ...
    department_id = models.CharField(max_length=255, help_text='QBO department id', null=True)
    transaction_date = models.DateField(help_text='Check transaction date')
    currency = models.CharField(max_length=5, help_text='Check Currency')
    private_note = models.TextField(help_text='Check Description')
    check_number = models.CharField(max_length=55, unique=True)
    created_at = models.DateTimeField(auto_now_add=True, help_text='Created at')
    updated_at = models.DateTimeField(auto_now=True, help_text='Updated at')

    @staticmethod
    def create_check(expense_group: ExpenseGroup):
        """
        Create check
        :param expense_group: expense group
        :return: check object
        ...
        general_mappings = GeneralMapping.objects.get(workspace_id=expense_group.workspace_id)
        check_object, _ = Cheque.objects.update_or_create(
            expense_group=expense_group,
            defaults={
                ...
                'check_number':  expense_group.fyle_group_id
            }
        )
        return check_object


class CheckLineitem(models.Model):
    """
    QBO Check Lineitem
    """
    id = models.AutoField(primary_key=True)
    cheque = models.ForeignKey(Cheque, on_delete=models.PROTECT, help_text='Reference to check')
    expense = models.OneToOneField(Expense, on_delete=models.PROTECT, help_text='Reference to Expense')
    account_id = models.CharField(max_length=255, help_text='QBO account id')
    class_id = models.CharField(max_length=255, help_text='QBO class id', null=True)
    customer_id = models.CharField(max_length=255, help_text='QBO customer id', null=True)
    amount = models.FloatField(help_text='Check amount')
    description = models.CharField(max_length=255, help_text='QBO check lineitem description', null=True)
    created_at = models.DateTimeField(auto_now_add=True, help_text='Created at')
    updated_at = models.DateTimeField(auto_now=True, help_text='Updated at')

    @staticmethod
    def create_check_lineitems(expense_group: ExpenseGroup):
        """
        Create check lineitems
        :param expense_group: expense group
        :return: lineitems objects
        """
        expenses = expense_group.expenses.all()
        ...
        for lineitem in expenses:
            ...
            check_lineitem_object, _ = CheckLineitem.objects.update_or_create(
                cheque=cheque,
                expense_id=lineitem.id,
                defaults={
                    'account_id': account.account_id if account else None,
            ...
            check_lineitem_objects.append(check_lineitem_object)

        return check_lineitem_objects
    ...
    def create_credit_card_purchase(expense_group: ExpenseGroup):
        ...
        credit_card_purchase_object, _ = CreditCardPurchase.objects.update_or_create(
            ...
            defaults={
                ...
                'employee_id': EmployeeMapping.objects.get(
                    employee_email=description.get('employee_email')).employee_id,
                'transaction_date': datetime.now().strftime("%Y-%m-%d"),
                'private_note': 'Report {0} / {1} exported on {2}'.format(
                    expense.claim_number, expense.report_id, datetime.now().strftime("%Y-%m-%d")
                ),
