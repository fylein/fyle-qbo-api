...
class QuickbooksCheck(models.Model):
    """
    QBO Check
    """
    id = models.AutoField(primary_key=True)
    ...
    def create_check(expense_group: ExpenseGroup):
        ...
        check_object, _ = QuickbooksCheck.objects.update_or_create(
            ...
            defaults={
                ...
                'check_number':  expense.report_id
            }
        )
        return check_object
...
class CheckLineitem(models.Model):
    ...
    """
    id = models.AutoField(primary_key=True)
    qbo_check = models.ForeignKey(QuickbooksCheck, on_delete=models.PROTECT, help_text='Reference to check')
    expense = models.OneToOneField(Expense, on_delete=models.PROTECT, help_text='Reference to Expense')
    account_id = models.CharField(max_length=255, help_text='QBO account id')
    class_id = models.CharField(max_length=255, help_text='QBO class id', null=True)
    customer_id = models.CharField(max_length=255, help_text='QBO customer id', null=True)
    ...
    def create_check_lineitems(expense_group: ExpenseGroup):
        ...
        """
        expenses = expense_group.expenses.all()
        qbo_check = QuickbooksCheck.objects.get(expense_group=expense_group)
