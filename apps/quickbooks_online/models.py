...
def get_expense_purpose(
    workspace_id, lineitem, category, workspace_general_settings
) -> str:
...
def get_ccc_account_id(
    workspace_general_settings, general_mappings, expense, description
):
    if workspace_general_settings.map_fyle_cards_qbo_account:
        ...
        ccc_account_id = (
            ccc_account_mapping.destination_card_account.destination_id
...
def get_class_id_or_none(expense_group: ExpenseGroup, lineitem: Expense):
    class_setting: MappingSetting = MappingSetting.objects.filter(
        workspace_id=expense_group.workspace_id, destination_field="CLASS"
    ).first()
    ...
    if class_setting:
        if class_setting.source_field == "PROJECT":
            ...
            attribute = ExpenseAttribute.objects.filter(
                attribute_type=class_setting.source_field
            ).first()
...
def get_customer_id_or_none(expense_group: ExpenseGroup, lineitem: Expense):
    ...
    if customer_setting:
        if customer_setting.source_field == "PROJECT":
            ...
            attribute = ExpenseAttribute.objects.filter(
                attribute_type=customer_setting.source_field
            ).first()
            source_value = lineitem.custom_properties.get(attribute.display_name, None)
...
def get_department_id_or_none(expense_group: ExpenseGroup, lineitem: Expense = None):
    department_setting: MappingSetting = MappingSetting.objects.filter(
        workspace_id=expense_group.workspace_id, destination_field="DEPARTMENT"
    ).first()
    ...
    if department_setting:
        if lineitem:
            if department_setting.source_field == "PROJECT":
                ...
                attribute = ExpenseAttribute.objects.filter(
                    attribute_type=department_setting.source_field
                ).first()
            ...
            source_value = (
                expense_group.description[department_setting.source_field.lower()]
                if department_setting.source_field.lower() in expense_group.description
                else None
            )
...
def get_category_mapping_and_detail_type(
...
class Bill(models.Model):
    ...
    accounts_payable_id = models.CharField(
        max_length=255, help_text="QBO Accounts Payable account id"
    )
    vendor_id = models.CharField(max_length=255, help_text="QBO vendor id")
    department_id = models.CharField(
        max_length=255, help_text="QBO department id", null=True
    )
...
class BillLineitem(models.Model):
    ...
    customer_id = models.CharField(
        max_length=255, help_text="QBO customer id", null=True
    )
    ...
    def create_bill_lineitems(
        ...
        for lineitem in expenses:
            ...
            account, detail_type = get_category_mapping_and_detail_type(
                workspace_general_settings, category, expense_group.workspace_id
            )
            ...
            bill_lineitem_object, _ = BillLineitem.objects.update_or_create(
                ...
                defaults={
                    ...
                    "item_id": account.destination.destination_id
...
class ChequeLineitem(models.Model):
    ...
    cheque = models.ForeignKey(
        Cheque, on_delete=models.PROTECT, help_text="Reference to cheque"
    )
    expense = models.OneToOneField(
        Expense, on_delete=models.PROTECT, help_text="Reference to Expense"
    )
    ...
    description = models.TextField(
        help_text="QBO cheque lineitem description", null=True
    )
    detail_type = models.CharField(
        max_length=255,
        help_text="Detail type for the lineitem",
...
class QBOExpense(models.Model):
    ...
    def create_qbo_expense(expense_group: ExpenseGroup):
        ...
        if (
            workspace_general_settings.map_merchant_to_vendor
            ...
            expense_group.description["spent_at"] = expense.spent_at.strftime(
                "%Y-%m-%d"
            )
            expense_group.save()
            ...
            if not entity:
                entity_id = (
                    DestinationAttribute.objects.filter(
                        value="Debit Card Misc", workspace_id=expense_group.workspace_id
                    )
                    .first()
                    .destination_id
                )
            ...
            entity_id = (
                entity.destination_employee.destination_id
...
class QBOExpenseLineitem(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    qbo_expense = models.ForeignKey(
        QBOExpense, on_delete=models.PROTECT, help_text="Reference to QBO Expense"
    )
    expense = models.OneToOneField(
        Expense, on_delete=models.PROTECT, help_text="Reference to Fyle Expense"
    )
    account_id = models.CharField(max_length=255, help_text="QBO account id", null=True)
    class_id = models.CharField(max_length=255, help_text="QBO class id", null=True)
    customer_id = models.CharField(
    ...
    @staticmethod
    def create_qbo_expense_lineitems(
        expense_group: ExpenseGroup,
        workspace_general_settings: WorkspaceGeneralSettings,
    ):
        """
        ...
        for lineitem in expenses:
            ...
            (
                qbo_expense_lineitem_object,
                _,
            ) = QBOExpenseLineitem.objects.update_or_create(
...
class CreditCardPurchase(models.Model):
    ...
    def create_credit_card_purchase(
        expense_group: ExpenseGroup, map_merchant_to_vendor: bool
    ):
        """
        ...
        ccc_account_id = get_ccc_account_id(
            workspace_general_settings, general_mappings, expense, description
        )
        ...
        credit_card_purchase_object, _ = CreditCardPurchase.objects.update_or_create(
            ...
            defaults={
                ...
                "private_note": private_note,
                "currency": expense.currency,
                "credit_card_purchase_number": expense.expense_number
                if map_merchant_to_vendor
                else "",
            },
        )
...
class CreditCardPurchaseLineitem(models.Model):
    ...
    billable = models.BooleanField(null=True, help_text="Expense Billable or not")
    description = models.TextField(
        help_text="QBO credit card purchase lineitem description", null=True
    )
    detail_type = models.CharField(
        max_length=255,
        help_text="Detail type for the lineitem",
    ...
    def create_credit_card_purchase_lineitems(
        ...
        """
        expenses: List[Expense] = expense_group.expenses.all()
        credit_card_purchase = CreditCardPurchase.objects.get(
            expense_group=expense_group
        )
        ...
        for lineitem in expenses:
            ...
            (
                credit_card_purchase_lineitem_object,
                _,
            ) = CreditCardPurchaseLineitem.objects.update_or_create(
            ...
            credit_card_purchase_lineitem_objects.append(
                credit_card_purchase_lineitem_object
            )
...
class JournalEntryLineitem(models.Model):
    ...
    journal_entry = models.ForeignKey(
        JournalEntry, on_delete=models.PROTECT, help_text="Reference to JournalEntry"
    )
    expense = models.OneToOneField(
        Expense, on_delete=models.PROTECT, help_text="Reference to Expense"
    )
    debit_account_id = models.CharField(
        max_length=255, help_text="QBO Debit account id"
    )
    account_id = models.CharField(max_length=255, help_text="QBO account id")
    class_id = models.CharField(max_length=255, help_text="QBO class id", null=True)
    entity_id = models.CharField(max_length=255, help_text="QBO entity id")
    entity_type = models.CharField(
        max_length=255, help_text="QBO Entity Type ( Vendor / Employee )"
    )
    customer_id = models.CharField(
        max_length=255, help_text="QBO customer id", null=True
    )
    ...
    posting_type = models.CharField(
        max_length=255, help_text="QBO posting type", null=True
    )
    amount = models.FloatField(help_text="JournalEntry amount")
    tax_amount = models.FloatField(null=True, help_text="Tax amount")
    tax_code = models.CharField(max_length=255, help_text="Tax Group ID", null=True)
    description = models.TextField(
        help_text="QBO JournalEntry lineitem description", null=True
    )
    created_at = models.DateTimeField(auto_now_add=True, help_text="Created at")
    updated_at = models.DateTimeField(auto_now=True, help_text="Updated at")
    ...
    def create_journal_entry_lineitems(
        ...
        for lineitem in expenses:
            ...
            if expense_group.fund_source == "PERSONAL":
                ...
                debit_account_id = get_ccc_account_id(
                    workspace_general_settings, general_mappings, lineitem, description
                )
            ...
            (
                journal_entry_lineitem_object,
                _,
            ) = JournalEntryLineitem.objects.update_or_create(
                ...
                defaults={
                    ...
                    "class_id": class_id,
                    "entity_id": entity.destination_employee.destination_id
                    if employee_field_mapping == "EMPLOYEE"
                    else entity.destination_vendor.destination_id,
...
class BillPayment(models.Model):
    ...
    def create_bill_payment(expense_group: ExpenseGroup):
        ...
        bill_payment_object, _ = BillPayment.objects.update_or_create(
            expense_group=expense_group,
            defaults={
                "private_note": "Payment for Bill by {0}".format(
                    description.get("employee_email")
                ),
                "vendor_id": vendor_id,
                "amount": total_amount,
                "currency": expense.currency,
...
class BillPaymentLineitem(models.Model):
    ...
    bill_payment = models.ForeignKey(
        BillPayment, on_delete=models.PROTECT, help_text="Reference to bill payment"
    )
    amount = models.FloatField(help_text="Bill amount")
    linked_transaction_id = models.CharField(
        max_length=255, help_text="Linked Transaction ID ( Bill ID )"
    )
    created_at = models.DateTimeField(auto_now_add=True, help_text="Created at")
    updated_at = models.DateTimeField(auto_now=True, help_text="Updated at")
    ...
    @staticmethod
    def create_bill_payment_lineitems(
        expense_group: ExpenseGroup, linked_transaction_id
    ):
        """
