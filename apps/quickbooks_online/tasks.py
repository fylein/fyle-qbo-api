...
def get_or_create_credit_card_or_debit_card_vendor(
    workspace_id: int,
    merchant: str,
    debit_card_expense: bool,
    ...
    try:
        ...
        try:
            ...
            entity = DestinationAttribute.objects.filter(
                workspace_id=expense_group.workspace_id, **filters
            ).first()
            ...
            if entity:
                ...
                if existing_employee_mapping:
                    ...
                    ] = existing_employee_mapping.destination_employee_id
                    destination[
                        "destination_card_account_id"
                    ] = existing_employee_mapping.destination_card_account_id
...
def create_bill(expense_group, task_log_id, last_export: bool):
    ...
    qbo_credentials = QBOCredential.get_active_qbo_credentials(
        expense_group.workspace_id
    )
    ...
    if (
        ...
        and general_settings.auto_map_employees
        ...
        create_or_update_employee_mapping(
            expense_group, qbo_connection, general_settings.auto_map_employees
        )
    ...
    with transaction.atomic():
        ...
        bill_lineitems_objects = BillLineitem.create_bill_lineitems(
...
def __validate_general_mapping(
    ...
    if (
        ...
        if general_mapping:
            if not (
                ...
                or general_mapping.default_ccc_vendor_name
            ):
                bulk_errors.append(
                    {
                        "row": None,
    ...
    if general_mapping and not (
        general_mapping.accounts_payable_id or general_mapping.accounts_payable_name
    ):
        if (
            general_settings.reimbursable_expenses_object == "BILL"
            or general_settings.corporate_credit_card_expenses_object == "BILL"
        ) or (
            general_settings.reimbursable_expenses_object == "JOURNAL ENTRY"
            and general_settings.employee_field_mapping == "VENDOR"
            and expense_group.fund_source == "PERSONAL"
        ):
            bulk_errors.append(
                {
                    "row": None,
    ...
    if (
        ...
        and (
            (
                ...
                or (
                    general_settings.reimbursable_expenses_object == "JOURNAL ENTRY"
                    and general_settings.employee_field_mapping == "EMPLOYEE"
                    and expense_group.fund_source == "PERSONAL"
                )
            )
        )
    ...
    if (
        ...
        and not (
            ...
            or general_mapping.qbo_expense_account_name
        )
...
def __validate_general_settings(
    expense_group: ExpenseGroup, general_settings: WorkspaceGeneralSettings
):
    bulk_errors = []
    ...
    if (
        ...
        or general_settings.corporate_credit_card_expenses_object == "JOURNAL ENTRY"
    ):
    ...
    if (
        general_settings.corporate_credit_card_expenses_object
        not in ("BILL", "DEBIT CARD EXPENSE")
        and expense_group.fund_source == "CCC"
    ):
        if not (
            general_mapping.default_ccc_account_id
            or general_mapping.default_ccc_account_name
        ):
            bulk_errors.append(
                {
                    "row": None,
    ...
    if (
        ...
        if not (
            ...
            or general_mapping.default_debit_card_account_name
        ):
            bulk_errors.append(
                {
                    "row": None,
    ...
    if general_settings.import_tax_codes and not (
    ...
    if not (
        ...
        and (
            (
                ...
                and general_settings.map_merchant_to_vendor
            )
            or general_settings.corporate_credit_card_expenses_object == "BILL"
        )
    ):
        ...
        try:
            ...
            if general_settings.employee_field_mapping == "EMPLOYEE":
                ...
                entity = (
                    entity.destination_vendor
                    if entity.destination_vendor and entity.destination_vendor.active
                    else None
                )
...
def __validate_expense_attributes(
    expense_group: ExpenseGroup, general_settings: WorkspaceGeneralSettings
):
    bulk_errors = []
    row = 0
...
def __validate_expense_group(
    ...
    expense_attibutes_errors = __validate_expense_attributes(
        expense_group, general_settings
    )

    bulk_errors = list(
        itertools.chain(
            general_mapping_errors, general_settings_errors, expense_attibutes_errors
        )
    )
...
def create_cheque(expense_group, task_log_id, last_export: bool):
    ...
    with transaction.atomic():
        ...
        cheque_line_item_objects = ChequeLineitem.create_cheque_lineitems(
            expense_group, general_settings
        )

        created_cheque = qbo_connection.post_cheque(
            cheque_object, cheque_line_item_objects
        )
        ...
        load_attachments(
            qbo_connection, created_cheque["Purchase"]["Id"], "Purchase", expense_group
        )
...
def create_qbo_expense(expense_group, task_log_id, last_export: bool):
    ...
    if expense_group.fund_source == "PERSONAL":
        ...
        get_or_create_credit_card_or_debit_card_vendor(
            expense_group.workspace_id, merchant, True, general_settings
        )
    ...
    with transaction.atomic():
        ...
        created_qbo_expense = qbo_connection.post_qbo_expense(
            qbo_expense_object, qbo_expense_line_item_objects
        )
    ...
    load_attachments(
        qbo_connection, created_qbo_expense["Purchase"]["Id"], "Purchase", expense_group
    )
...
def create_credit_card_purchase(
    expense_group: ExpenseGroup, task_log_id, last_export: bool
):
    task_log = TaskLog.objects.get(id=task_log_id)
    if task_log.status not in ["IN_PROGRESS", "COMPLETE"]:
        task_log.status = "IN_PROGRESS"
    ...
    if not general_settings.map_merchant_to_vendor:
        ...
        merchant = expense_group.expenses.first().vendor
        get_or_create_credit_card_or_debit_card_vendor(
            expense_group.workspace_id, merchant, False, general_settings
        )
    ...
    with transaction.atomic():
        ...
        credit_card_purchase_lineitems_objects = (
            CreditCardPurchaseLineitem.create_credit_card_purchase_lineitems(
                expense_group, general_settings
            )
        )
        ...
        load_attachments(
            qbo_connection,
            created_credit_card_purchase["Purchase"]["Id"],
            "Purchase",
            expense_group,
        )
...
def create_journal_entry(expense_group, task_log_id, last_export: bool):
    ...
    with transaction.atomic():
        ...
        created_journal_entry = qbo_connection.post_journal_entry(
            ...
            journal_entry_lineitems_objects,
            general_settings.je_single_credit_line,
        )
    ...
    load_attachments(
        qbo_connection,
        created_journal_entry["JournalEntry"]["Id"],
        "JournalEntry",
        expense_group,
    )
...
def process_bill_payments(bill: Bill, workspace_id: int, task_log: TaskLog):
    ...
    with transaction.atomic():
        ...
        bill_payment_lineitems_objects = (
            BillPaymentLineitem.create_bill_payment_lineitems(
...
def check_qbo_object_status(workspace_id):
    try:
        ...
        if bills:
            ...
            for bill in bills:
                bill_object = qbo_connection.get_bill(
                    bill_ids[bill.expense_group.id]["qbo_object_id"]
                )
