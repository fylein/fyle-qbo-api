...
def handle_quickbooks_error(
    exception, expense_group: ExpenseGroup, task_log: TaskLog, export_type: str
):
    ...
    if "Fault" not in response:
        ...
        for error in quickbooks_errors:
            error = {
                ...
                if error["Message"]
                else "{0} error".format(export_type),
    ...
    def decorator(func):
        def new_fn(*args):
            ...
            try:
                ...
                logger.info(
                    "QBO Account not connected / token expired for workspace_id %s / \
                        expense group %s",
