...
class Expense(models.Model):
    ...
    foreign_currency = models.CharField(
        null=True, max_length=15, help_text='Foreign Currency'
    )
    ...
    settlement_id = models.CharField(
        max_length=255, null=True, help_text='Settlement ID'
    )
    ...
    paid_on_qbo = models.BooleanField(
        help_text='Expense Payment status on QBO', default=False
    )
    payment_number = models.CharField(
        max_length=55, help_text='Expense payment number', null=True
    )
    is_skipped = models.BooleanField(
        null=True, default=False, help_text='Expense is skipped or not'
    )
...
class ExpenseGroupSettings(models.Model):
    ...
    ccc_export_date_type = models.CharField(
        max_length=100, default='current_date', help_text='CCC Export Date'
    )
...
class ExpenseGroup(models.Model):
    ...
    def create_expense_groups_by_report_id_fund_source(
        ...
        if (
            ...
            and 'expense_id' not in reimbursable_expense_group_fields
        ):
            total_amount = 0
            ...
            if total_amount < 0:
                reimbursable_expenses = list(
                    filter(lambda expense: expense.amount > 0, reimbursable_expenses)
                )
...
class ExpenseFilter(models.Model):
    ...
    operator = models.CharField(
        max_length=255,
        choices=EXPENSE_FILTER_OPERATOR,
    ...
    rank = models.IntegerField(
        choices=EXPENSE_FILTER_RANK, help_text='Rank for the filter'
    )
