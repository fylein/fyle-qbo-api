...
class Workspace(models.Model):
    ...
    cluster_domain = models.CharField(
        max_length=255, help_text="fyle cluster domain", null=True
    )
...
class WorkspaceGeneralSettings(models.Model):
    ...
    employee_field_mapping = models.CharField(
        max_length=50, help_text="Mapping Settings ( VENDOR / EMPLOYEE )"
    )
    map_merchant_to_vendor = models.BooleanField(
        help_text="Map Merchant to Vendor for CCC Expenses", default=False
    )
    import_categories = models.BooleanField(
        default=False, help_text="Auto import Categories to Fyle"
    )
    import_items = models.BooleanField(
        default=False, help_text="Auto import Items to Fyle"
    )
    import_projects = models.BooleanField(
        default=False, help_text="Auto import projects to Fyle"
    )
    ...
    change_accounting_period = models.BooleanField(
        default=False, help_text="Export Expense when accounting period is closed"
    )
    ...
    memo_structure = ArrayField(
        base_field=models.CharField(max_length=100),
        default=get_default_memo_fields,
    ...
    auto_create_merchants_as_vendors = models.BooleanField(
        default=False, help_text="Auto create Fyle Merchants as QBO vendors"
    )
    sync_fyle_to_qbo_payments = models.BooleanField(
        default=False, help_text="Auto Sync Payments from Fyle to QBO"
    )
    sync_qbo_to_fyle_payments = models.BooleanField(
    ...
    category_sync_version = models.CharField(
        default="v2", max_length=50, help_text="Category sync version"
    )
    je_single_credit_line = models.BooleanField(
        default=False, help_text="Single Credit Line for Journal Entries"
    )
    map_fyle_cards_qbo_account = models.BooleanField(
    ...
    import_vendors_as_merchants = models.BooleanField(
        default=False, help_text="Auto import vendors from qbo as merchants to Fyle"
    )
    is_multi_currency_allowed = models.BooleanField(
...
class QBOCredential(models.Model):
    ...
    realm_id = models.CharField(
        max_length=40, help_text="QBO realm / company Id", null=True
    )
    is_expired = models.BooleanField(default=False, help_text="QBO token expiry flag")
    company_name = models.CharField(
        max_length=255, help_text="QBO Company Name", null=True
    )
    ...
    def get_active_qbo_credentials(workspace_id):
        return QBOCredential.objects.get(
