        ...
        try:
            ...
            if not qbo_credentials:
                qbo_credentials = QBOCredential.objects.create(
                    refresh_token=refresh_token, realm_id=realm_id, workspace=workspace
                )
            ...
            qbo_connector = QBOConnector(
                qbo_credentials, workspace_id=kwargs["workspace_id"]
            )
            ...
            qbo_credentials.currency = preferences["CurrencyPrefs"]["HomeCurrency"][
                "value"
            ]
    ...
    def patch(self, request, **kwargs):
        ...
        try:
            revoke_refresh_token(
                refresh_token, settings.QBO_CLIENT_ID, settings.QBO_CLIENT_SECRET
            )
        ...
        return Response(
            data={"workspace_id": workspace_id, "message": "QBO Refresh Token deleted"}
        )
...
class GeneralSettingsView(viewsets.ViewSet):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            data=self.serializer_class(general_settings).data, status=status.HTTP_200_OK
        )
    ...
    def patch(self, request, **kwargs):
        ...
        """
        workspace_general_settings_object = WorkspaceGeneralSettings.objects.get(
            workspace_id=kwargs["workspace_id"]
        )
...
class LastExportDetailView(viewsets.ViewSet):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            last_export_detail.last_exported_at
            and last_export_detail.total_expense_groups_count
        ):
            return Response(
            ...
            return Response(
                data={"message": "latest exported details does not exist in workspace"},
        ...
        try:
            ...
            if "fyle for" in workspace.name.lower():
                ...
                for healthy_token in healthy_tokens:
                    logger.info(
                        "Checking token health for workspace: {}".format(
                            healthy_token.workspace_id
                        )
                    )
                    # Token Health check
                    try:
                        qbo_connector = QBOConnector(
                            healthy_token, workspace_id=workspace.id
                        )
                        qbo_connector.get_company_preference()
                        logger.info(
                            "Yaay, token is healthly for workspace: {}".format(
                                healthy_token.workspace_id
                            )
                        )
                    except Exception:
                        ...
                        logger.info(
                            "Oops, token is dead for workspace: {}".format(
                                healthy_token.workspace_id
                            )
                        )
                    ...
                    with transaction.atomic():
                        if healthy_token:
                            ...
                            with connection.cursor() as cursor:
                                cursor.execute(
                                    "select reset_workspace(%s)", [workspace.id]
                                )
            ...
            return Response(
                status=status.HTTP_400_BAD_REQUEST, data={"message": error_message}
            )
