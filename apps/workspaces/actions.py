...
def connect_qbo_oauth(refresh_token, realm_id, workspace_id):
    ...
    # Get the QBO credentials associated with the workspace,
    # or create new credentials if none exist
    ...
    return Response(
        data=QBOCredentialSerializer(qbo_credentials).data, status=status.HTTP_200_OK
    )
        ...
        if "fyle for" in workspace.name.lower():
            ...
            for healthy_token in healthy_tokens:
                ...
                try:
                    ...
                    # If the token is expired, setting is_expired = True so that they
                    # are not used for future runs
                    logger.info(
