...
class ImportSettingsTrigger:
    ...
    def remove_department_grouping(self, source_field: str):
        ...
        if workspace_general_settings.corporate_credit_card_expenses_object:
            corporate_credit_card_settings = list(
    ...
    def __remove_old_department_source_field(
        ...
        """
        Should remove Department Source field from Reimbursable settings
        in case of deletion and updation
        """
        old_department_setting = current_mappings_settings.filter(
            destination_field="DEPARTMENT"
        ).first()

        new_department_setting = list(
            filter(
                lambda setting: setting["destination_field"] == "DEPARTMENT",
        ...
        if (
            old_department_setting
            and new_department_setting
            and old_department_setting.source_field
            != new_department_setting[0]["source_field"]
        ):
    ...
    def pre_save_mapping_settings(self):
        ...
        current_mapping_settings = MappingSetting.objects.filter(
            workspace_id=self.__workspace_id
        ).all()
