...
class ExpenseGroupSettingsSerializer(serializers.ModelSerializer):
    reimbursable_expense_group_fields = serializers.ListField(
        allow_null=True, required=False
    )
    reimbursable_export_date_type = serializers.CharField(
        allow_null=True, allow_blank=True, required=False
    )
    corporate_credit_card_expense_group_fields = serializers.ListField(
        allow_null=True, required=False
    )
    ccc_export_date_type = serializers.CharField(
        allow_null=True, allow_blank=True, required=False
    )
    ccc_expense_state = serializers.CharField(
        allow_null=True, allow_blank=True, required=False
    )
    expense_state = serializers.CharField(
        allow_null=True, allow_blank=True, required=False
    )
...
class ExportSettingsSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated):
        ...
        workspace_general_settings_instance = WorkspaceGeneralSettings.objects.get(
        ...
        if workspace_general_settings_instance.map_merchant_to_vendor:
            map_merchant_to_vendor = (
                workspace_general_settings_instance.map_merchant_to_vendor
            )

        category_sync_version = (
            workspace_general_settings_instance.category_sync_version
        ...
        if workspace_general_settings.get('corporate_credit_card_expenses_object') and (
            workspace_general_settings.get('corporate_credit_card_expenses_object')
        ...
        WorkspaceGeneralSettings.objects.update_or_create(
            workspace=instance,
            defaults={
                'reimbursable_expenses_object': workspace_general_settings.get(
                    'reimbursable_expenses_object'
                ),
        ...
        if (
            ...
            or (
                map_merchant_to_vendor
                and workspace_general_settings['corporate_credit_card_expenses_object']
                == 'CREDIT CARD PURCHASE'
            )
            or (
                workspace_general_settings.get('reimbursable_expenses_object')
                in ('JOURNAL ENTRY', 'EXPENSE')
            )
        ):
            expense_group_settings['import_card_credits'] = True

        ExpenseGroupSettings.update_expense_group_settings(
            expense_group_settings, instance.id
        )

        GeneralMapping.objects.update_or_create(
            workspace=instance,
            defaults={
                'accounts_payable_name': general_mappings.get('accounts_payable').get(
                    'name'
                ),
                'accounts_payable_id': general_mappings.get('accounts_payable').get(
                    'id'
                ),
                'qbo_expense_account_name': general_mappings.get(
                    'qbo_expense_account'
                ).get('name'),
                'qbo_expense_account_id': general_mappings.get(
                    'qbo_expense_account'
                ).get('id'),
                'bank_account_name': general_mappings.get('bank_account').get('name'),
                'bank_account_id': general_mappings.get('bank_account').get('id'),
                'default_ccc_account_name': general_mappings.get(
                    'default_ccc_account'
                ).get('name'),
                'default_ccc_account_id': general_mappings.get(
                    'default_ccc_account'
                ).get('id'),
                'default_debit_card_account_name': general_mappings.get(
                    'default_debit_card_account'
                ).get('name'),
                'default_debit_card_account_id': general_mappings.get(
                    'default_debit_card_account'
                ).get('id'),
                'default_ccc_vendor_name': general_mappings.get(
                    'default_ccc_vendor'
                ).get('name'),
                'default_ccc_vendor_id': general_mappings.get('default_ccc_vendor').get(
                    'id'
                ),
            },
        )
