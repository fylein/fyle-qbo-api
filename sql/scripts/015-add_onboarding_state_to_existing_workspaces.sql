...
select 
    ...
    qc.id as qbo_creds_id 
from workspaces w 
left join 
    workspace_general_settings wgs on w.id = wgs.workspace_id 
left join 
    qbo_credentials qc on qc.workspace_id = w.id 
left join 
...
where 
    configuration_id is not null and general_mappings_id is not null and qbo_creds_id is not null;
...
from all_settings_view 
where 
    configuration_id is not null and general_mappings_id is null and qbo_creds_id is not null;
...
from all_settings_view 
where 
    configuration_id is null and general_mappings_id is null and qbo_creds_id is not null;
...
where id in (
    ...
    from all_settings_view 
    where 
        configuration_id is null and general_mappings_id is not null and qbo_creds_id is not null
);
...
-- Count of all workspaces where qbo creds are not present, configuration is present and general mappings are present
select 
...
where id in (
    ...
    from all_settings_view 
    where 
        configuration_id is not null and general_mappings_id is null and qbo_creds_id is null
);
