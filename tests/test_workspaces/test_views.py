...
def test_post_of_workspace(mocker, api_client, test_connection):
    mocker.patch(
        ...
        return_value={
            "data": {
                "org": {"name": "Test Trip", "id": "orZu2yrz7zdy", "currency": "USD"}
            }
        },
    )
    url = reverse("workspace")
...
def test_post_of_new_workspace(mocker, api_client, test_connection):
    mocker.patch(
        ...
        return_value={
            "data": {
                "org": {"name": "Test Trip", "id": "orZu2y7zdy", "currency": "USD"}
            }
        },
    )
    url = reverse("workspace")
...
def test_get_configuration_detail(api_client, test_connection):
    ...
    url = reverse("workspace-general-settings", kwargs={"workspace_id": workspace_id})
    ...
    response = json.loads(response.content)
    assert (
        dict_compare_keys(response, data["general_settings"]) == []
    ), "configuration api returns a diff in keys"
...
def test_post_workspace_configurations(api_client, test_connection):
    ...
    response = api_client.post(
        url, data=data["workspace_general_settings_payload"], format="json"
    )
    assert response.status_code == 200
    ...
    response = api_client.post(url, data=updated_data, format="json")
    assert response.status_code == 200
...
def test_get_qbo_credentials_view(api_client, test_connection):
    url = reverse("get-qbo-credentials", kwargs={"workspace_id": 4})
...
def test_post_connect_qbo_view(mocker, api_client, test_connection):
    mocker.patch(
        "apps.workspaces.views.generate_qbo_refresh_token", return_value="asdfghjk"
    )
    ...
    mocker.patch(
        ...
        return_value={"CurrencyPrefs": {"HomeCurrency": {"value": "USD"}}},
    )
...
def test_connect_qbo_view_exceptions(api_client, test_connection):
    ...
    with mock.patch("apps.workspaces.views.generate_qbo_refresh_token") as mock_call:
        mock_call.side_effect = qbo_exc.UnauthorizedClientError(
            msg="Invalid Authorization Code", response="Invalid Authorization Code"
        )

        response = api_client.post(url, data={"code": code})
        assert response.status_code == 401

        mock_call.side_effect = qbo_exc.NotFoundClientError(
            msg="Fyle Application not found", response="Fyle Application not found"
        )

        response = api_client.post(url, data={"code": code})
        assert response.status_code == 404

        mock_call.side_effect = qbo_exc.WrongParamsError(
            msg="Some of the parameters are wrong",
            response="Some of the parameters are wrong",
        )

        response = api_client.post(url, data={"code": code})
        assert response.status_code == 500

        mock_call.side_effect = qbo_exc.InternalServerError(
...
def test_prepare_e2e_test_view(mock_db, mocker, api_client, test_connection):
    ...
    mocker.patch(
        "apps.workspaces.models.QBOCredential.objects.create", return_value=None
    )
