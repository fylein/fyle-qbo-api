...
def test_expense_group_view(api_client, test_connection):
    ...
    response = api_client.get(url, {"expense_group_ids": "1,2"})
    assert response.status_code == 200

    response = json.loads(response.content)
    assert response == {"count": 0, "next": None, "previous": None, "results": []}

    response = api_client.get(url, {"state": "ALL"})
    assert response.status_code == 200
    ...
    response = api_client.get(url, {"state": "READY"})

    response = json.loads(response.content)
    assert response == data["expense_groups_ready_response"]

    response = api_client.get(url, {"state": "FAILED"})
    response = json.loads(response.content)
    assert response == {"count": 0, "next": None, "previous": None, "results": []}
    ...
    response = api_client.post(url, data={"task_log_id": task_log.id}, format="json")
    assert response.status_code == 200
...
def test_employees_view(api_client, test_connection):
    ...
    url = reverse("employees", kwargs={"workspace_id": 3})
...
def test_expense_filters(api_client, test_connection):
    ...
    assert (
        dict_compare_keys(response, data["expense_filter_1_response"]) == []
    ), "expense group api return diffs in keys"
    ...
    assert (
        dict_compare_keys(response, data["expense_filter_2_response"]) == []
    ), "expense group api return diffs in keys"
    ...
    assert (
        dict_compare_keys(response, data["expense_filters_response"]) == []
    ), "expense group api return diffs in keys"
...
def test_custom_fields(mocker, api_client, test_connection):
    ...
    assert (
        dict_compare_keys(response, data["custom_fields_response"]) == []
    ), "expense group api return diffs in keys"
...
def test_expenses(mocker, api_client, test_connection):
    ...
    assert (
        dict_compare_keys(response, data["skipped_expenses"]) == []
    ), "expense group api return diffs in keys"
