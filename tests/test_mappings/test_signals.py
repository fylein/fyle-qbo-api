...
def test_resolve_post_employees_mapping_errors(test_connection):
    ...
    error = Error.objects.filter(
        expense_attribute_id=employee_mapping.source_employee_id
    ).first()
...
def test_run_post_mapping_settings_triggers(test_connection, mocker):
    ...
    schedule = Schedule.objects.filter(
        func='apps.mappings.tasks.auto_create_project_mappings', args='{}'.format(2)
    ).first()
    ...
    schedule = Schedule.objects.filter(
        func='apps.mappings.tasks.auto_create_cost_center_mappings', args='{}'.format(1)
    ).first()
    ...
    assert (
        'project' in expense_group_settings.corporate_credit_card_expense_group_fields
    )
...
def test_run_post_delete_mapping_settings_triggers(test_connection):
    ...
    assert (
        'cost_center'
        not in expense_group_settings.corporate_credit_card_expense_group_fields
    )
