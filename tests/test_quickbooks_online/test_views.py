...
def test_destination_attributes_view(api_client, test_connection):
    ...
    response = api_client.get(
        url, {"attribute_types": "ACCOUNT", "display_name": "Account"}
    )
...
def test_searched_destination_attributes_view(api_client, test_connection):
    ...
    response = api_client.get(
        url, {"attribute_type": "ACCOUNT", "display_name": "Account"}
    )
    assert response.status_code == 200
    response = json.loads(response.content)
...
def test_qbo_attributes_view(api_client, test_connection):
    ...
    response = api_client.get(url, {"attribute_types": "CUSTOMER"})
    assert response.status_code == 200
    response = json.loads(response.content)
...
def test_get_company_preference_exceptions(api_client, test_connection, db):
    ...
    with mock.patch(
        "apps.quickbooks_online.utils.QBOConnector.get_company_info"
    ) as mock_call:
        mock_call.side_effect = WrongParamsError(
            msg="wrong params", response="invalid_params"
        )
        response = api_client.get(url)
        assert response.status_code == 400

        mock_call.side_effect = InvalidTokenError(
            msg="Invalid token, try to refresh it",
            response="Invalid token, try to refresh it",
        )
        response = api_client.get(url)
        assert response.status_code == 400
...
def test_vendor_view(mocker, api_client, test_connection):
    mocker.patch(
        "apps.quickbooks_online.utils.QBOConnector.sync_vendors", return_value=None
    )
    ...
    vendor = DestinationAttribute.objects.filter(
        attribute_type="VENDOR", active=True, workspace_id=3
    ).first()
    vendor.active = False
    vendor.save()
...
def test_employee_view(mocker, api_client, test_connection):
    mocker.patch(
        "apps.quickbooks_online.utils.QBOConnector.sync_employees", return_value=None
    )
