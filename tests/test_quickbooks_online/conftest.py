...
def create_bill_payment(db):
    ...
    bill_payment_lineitems = BillPaymentLineitem.create_bill_payment_lineitems(
        expense_group, workspace_general_settings
    )
...
def create_task_logs(db):
    TaskLog.objects.update_or_create(
        workspace_id=3, type="FETCHING_EXPENSES", defaults={"status": "READY"}
    )

    TaskLog.objects.update_or_create(
        workspace_id=4, type="FETCHING_EXPENSES", defaults={"status": "READY"}
    )
