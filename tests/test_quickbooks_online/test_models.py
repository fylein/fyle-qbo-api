...
def test_qbo_expense(db):
    ...
    qbo_expense_lineitems = QBOExpenseLineitem.create_qbo_expense_lineitems(
        expense_group, workspace_general_settings
    )
...
def test_create_journal_entry(db):
    ...
    journal_entry_lineitems = JournalEntryLineitem.create_journal_entry_lineitems(
        expense_group, workspace_general_settings
    )
...
def test_create_bill_payment(db):
    ...
    bill_lineitems = BillPaymentLineitem.create_bill_payment_lineitems(
        expense_group, linked_transaction_id
    )
...
def test_create_credit_card_purchase(db):
    ...
    expense_group = ExpenseGroup.objects.get(id=17)
    credit_card_purchase = CreditCardPurchase.create_credit_card_purchase(
        expense_group, True
    )
    workspace_general_settings = WorkspaceGeneralSettings.objects.get(workspace_id=3)
...
def test_create_cheque(db):
    ...
    workspace_general_settings = WorkspaceGeneralSettings.objects.get(workspace_id=3)
    cheque_lineitems = ChequeLineitem.create_cheque_lineitems(
        expense_group, workspace_general_settings
    )
...
def test_get_expense_purpose():
    ...
    for lineitem in expenses:
        ...
        expense_purpose = get_expense_purpose(
            3, lineitem, category, workspace_general_settings
        )
...
def test_get_ccc_account_id():
    ...
    for lineitem in expenses:
        ccc_account_id = get_ccc_account_id(
            workspace_general_settings, general_mapping, lineitem, description
        )
...
def test_support_post_date_integrations(mocker, db):
    ...
    assert bill.transaction_date.strftime(
        "%m/%d/%Y"
