...
def test_sync_employees(mocker, db):
    ...
    employee_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='EMPLOYEE'
    ).count()
    assert employee_count == 2

    qbo_connection.sync_employees()

    new_employee_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='EMPLOYEE'
    ).count()
...
def test_post_vendor(mocker, db):
    ...
    vendor = qbo_connection.get_or_create_vendor(
        vendor_name='test Sharma', email='test@fyle.in', create=True
    )
...
def test_sync_vendors(mocker, db):
    ...
    vendor_count = DestinationAttribute.objects.filter(
        workspace_id=4, attribute_type='VENDOR'
    ).count()
    assert vendor_count == 47

    qbo_connection.sync_vendors()

    new_vendor_count = DestinationAttribute.objects.filter(
        workspace_id=4, attribute_type='VENDOR'
    ).count()
...
def test_sync_departments(mocker, db):
    mocker.patch(
        'qbosdk.apis.Departments.get', return_value=data['department_response']
    )
    qbo_credentials = QBOCredential.get_active_qbo_credentials(3)
    qbo_connection = QBOConnector(credentials_object=qbo_credentials, workspace_id=3)

    department_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='DEPARTMENT'
    ).count()
    assert department_count == 0

    qbo_connection.sync_departments()

    new_department_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='DEPARTMENT'
    ).count()
...
def test_sync_items(mocker, db):
    ...
    with mock.patch('qbosdk.apis.Items.get') as mock_call:
        ...
        qbo_connection = QBOConnector(
            credentials_object=qbo_credentials, workspace_id=3
        )

        item_count = DestinationAttribute.objects.filter(
            workspace_id=3, attribute_type='ACCOUNT', display_name='Item'
        ).count()
        assert item_count == 0

        qbo_connection.sync_items()

        new_item_count = DestinationAttribute.objects.filter(
            workspace_id=3, attribute_type='ACCOUNT', display_name='Item', active=True
        ).count()
        assert new_item_count == 0

        WorkspaceGeneralSettings.objects.filter(workspace_id=3).update(
            import_items=True
        )
        qbo_connection.sync_items()
        new_item_count = DestinationAttribute.objects.filter(
        ...
        active_item_count = DestinationAttribute.objects.filter(
            workspace_id=3, attribute_type='ACCOUNT', display_name='Item', active=True
        ).count()
        assert active_item_count == 2
        inactive_item_count = DestinationAttribute.objects.filter(
            workspace_id=3, attribute_type='ACCOUNT', display_name='Item', active=False
        ).count()
...
def test_construct_bill(create_bill, mocker, db):
    mocker.patch(
        'qbosdk.apis.ExchangeRates.get_by_source', return_value={'Rate': 1.2309}
    )
    qbo_credentials = QBOCredential.get_active_qbo_credentials(3)
    qbo_connection = QBOConnector(credentials_object=qbo_credentials, workspace_id=3)
    ...
    bill_object = qbo_connection._QBOConnector__construct_bill(
        bill=bill, bill_lineitems=bill_lineitems
    )
    data['bill_payload']['TxnDate'] = bill_object['TxnDate']

    assert (
        dict_compare_keys(bill_object, data['bill_payload']) == []
    ), 'construct bill_payload entry api return diffs in keys'
...
def test_construct_bill_item_based(create_bill_item_based, mocker, db):
    ...
    assert (
        dict_compare_keys(bill_object, data['bill_payload_item_based_payload']) == []
    ), 'construct bill_payload entry api return diffs in keys'


def test_construct_bill_item_and_account_based(
    create_bill_item_and_account_based, mocker, db
):
    mocker.patch(
        'qbosdk.apis.ExchangeRates.get_by_source', return_value={'Rate': 1.2309}
    )
    ...
    assert (
        dict_compare_keys(
            bill_object, data['bill_payload_item_and_account_based_payload']
        )
        == []
    ), 'construct bill_payload entry api return diffs in keys'
...
def test_construct_credit_card_purchase(create_credit_card_purchase, db):
    ...
    credit_crad_purchase_object = (
        qbo_connection._QBOConnector__construct_credit_card_purchase(
            credit_card_purchase=credit_card_purchase,
            credit_card_purchase_lineitems=credit_card_purchase_lineitems,
        )
    )

    data['credit_card_purchase_payload']['TxnDate'] = credit_crad_purchase_object[
        'TxnDate'
    ]

    assert (
        dict_compare_keys(
            credit_crad_purchase_object, data['credit_card_purchase_payload']
        )
        == []
    ), 'construct credit_card_purchase_payload entry api return diffs in keys'


def test_construct_credit_card_purchase_item_based(
    create_credit_card_purchase_item_based, db
):
    qbo_credentials = QBOCredential.get_active_qbo_credentials(3)
    qbo_connection = QBOConnector(credentials_object=qbo_credentials, workspace_id=3)
    ...
    ) = create_credit_card_purchase_item_based
    credit_crad_purchase_object = (
        qbo_connection._QBOConnector__construct_credit_card_purchase(
            credit_card_purchase=credit_card_purchase,
            credit_card_purchase_lineitems=credit_card_purchase_lineitems,
    ...
    assert (
        dict_compare_keys(
            credit_crad_purchase_object, data['credit_card_purchase_item_based_payload']
        )
        == []
    ), 'construct credit_card_purchase_payload entry api return diffs in keys'


def test_construct_credit_card_purchase_item_and_account_based(
    create_credit_card_purchase_item_and_account_based, db
):
    qbo_credentials = QBOCredential.get_active_qbo_credentials(3)
    qbo_connection = QBOConnector(credentials_object=qbo_credentials, workspace_id=3)
    ...
    (
        credit_card_purchase,
        credit_card_purchase_lineitems,
    ) = create_credit_card_purchase_item_and_account_based
    credit_crad_purchase_object = (
        qbo_connection._QBOConnector__construct_credit_card_purchase(
            credit_card_purchase=credit_card_purchase,
            credit_card_purchase_lineitems=credit_card_purchase_lineitems,
    ...
    credit_crad_purchase_object['Line'][0]['DetailType'] == 'ItemBasedExpenseLineDetail'
    credit_crad_purchase_object['Line'][1][
        'DetailType'
    ...
    assert (
        dict_compare_keys(
            credit_crad_purchase_object,
            data['credit_card_purchase_item_and_account_based_payload'],
        )
        == []
    ), 'construct credit_card_purchase_payload entry api return diffs in keys'
...
def test_construct_journal_entry(create_journal_entry, db):
    ...
    journal_entry_object = qbo_connection._QBOConnector__construct_journal_entry(
        journal_entry=journal_entry,
        journal_entry_lineitems=journal_entry_lineitems,
        single_credit_line=False,
    )

    assert (
        dict_compare_keys(journal_entry_object, data['journal_entry_payload']) == []
    ), 'construct journal entry api return diffs in keys'
...
def test_construct_qbo_expense(create_qbo_expense, db):
    ...
    qbo_expense_object = qbo_connection._QBOConnector__construct_qbo_expense(
        qbo_expense=qbo_expense, qbo_expense_lineitems=qbo_expense_lineitems
    )

    data['qbo_expense_payload']['TxnDate'] = qbo_expense_object['TxnDate']

    assert (
        dict_compare_keys(qbo_expense_object, data['qbo_expense_payload']) == []
    ), 'construct expense api return diffs in keys'
...
def test_construct_qbo_expense_item_based(create_qbo_expense_item_based, db):
    ...
    assert (
        dict_compare_keys(qbo_expense_object, data['qbo_expense_item_based_payload'])
        == []
    ), 'construct expense api return diffs in keys'


def test_construct_qbo_expense_item_and_account_based(
    create_qbo_expense_item_and_account_based, db
):
    qbo_credentials = QBOCredential.get_active_qbo_credentials(3)
    qbo_connection = QBOConnector(credentials_object=qbo_credentials, workspace_id=3)
    ...
    assert (
        dict_compare_keys(
            qbo_expense_object, data['qbo_expense_item_and_account_based_payload']
        )
        == []
    ), 'construct expense api return diffs in keys'
...
def test_construct_cheque(create_cheque, db):
    ...
    cheque_object = qbo_connection._QBOConnector__construct_cheque(
        cheque=cheque, cheque_lineitems=cheque_lineitems
    )

    data['cheque_payload']['TxnDate'] = cheque_object['TxnDate']

    assert (
        dict_compare_keys(cheque_object, data['cheque_payload']) == []
    ), 'construct cheque api return diffs in keys'
...
def test_construct_cheque_item_based(create_cheque_item_based, db):
    ...
    assert (
        dict_compare_keys(cheque_object, data['cheque_item_based_payload']) == []
    ), 'construct cheque api return diffs in keys'


def test_construct_cheque_item_and_account_based(
    create_cheque_item_and_account_based, db
):
    qbo_credentials = QBOCredential.get_active_qbo_credentials(3)
    qbo_connection = QBOConnector(credentials_object=qbo_credentials, workspace_id=3)
    ...
    assert (
        dict_compare_keys(cheque_object, data['cheque_item_and_account_based_payload'])
        == []
    ), 'construct cheque api return diffs in keys'
...
def test_get_effective_tax_rates(mocker, db):
    mocker.patch(
        'qbosdk.apis.TaxRates.get_by_id', return_value=data['tax_rate_get_by_id']
    )
    qbo_credentials = QBOCredential.get_active_qbo_credentials(3)
    qbo_connection = QBOConnector(credentials_object=qbo_credentials, workspace_id=3)
    effective_tax_rate, tax_rate_refs = qbo_connection.get_effective_tax_rates(
        [
            {
                'TaxRateRef': {'value': '5', 'name': 'NO TAX PURCHASE'},
...
def test_sync_tax_codes(mocker, db):
    ...
    tax_code_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='TAX_CODE'
    ).count()
...
def test_sync_dimensions(mocker, db):
    ...
    employee_count = DestinationAttribute.objects.filter(
        attribute_type='EMPLOYEE', workspace_id=1
    ).count()
    accounts_count = DestinationAttribute.objects.filter(
        attribute_type='ACCOUNT', workspace_id=1
    ).count()
    vendors_count = DestinationAttribute.objects.filter(
        attribute_type='VENDOR', workspace_id=1
    ).count()
...
def test_sync_classes(mocker, db):
    ...
    class_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='CLASS'
    ).count()
    assert class_count == 0

    qbo_connection.sync_classes()

    new_class_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='CLASS'
    ).count()
...
def test_sync_customers(mocker, db):
    ...
    customer_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='CUSTOMER'
    ).count()
    assert customer_count == 29

    qbo_connection.sync_customers()

    new_customer_count = DestinationAttribute.objects.filter(
        workspace_id=3, attribute_type='CUSTOMER'
    ).count()
...
def test_post_bill_exception(mocker, db, create_bill):
    ...
    mocker.patch(
        'qbosdk.apis.Preferences.get', return_value=data['preference_response']
    )
    workspace_id = 4

    qbo_credentials = QBOCredential.get_active_qbo_credentials(workspace_id)
    qbo_connection = QBOConnector(
        credentials_object=qbo_credentials, workspace_id=workspace_id
    )
...
def tests_post_bill_payment(mocker, db, create_bill_payment):
    mocker.patch(
        'qbosdk.apis.BillPayments.post', return_value=data['bill_payment_response']
    )
    workspace_id = 3
    ...
    created_bill_payment = qbo_connection.post_bill_payment(
        bill_payment, bill_payment_lineitems
    )
    assert (
        dict_compare_keys(created_bill_payment, data['bill_payment_response']) == []
    ), 'construct bill payment api return diffs in keys'
...
def test_post_attachments(mocker, db):
    ...
    qbo_connection.post_attachments(
        'asd',
        'dfg',
        [{'download_url': 'sdfghj', 'name': 'ert', 'content_type': 'application/pdf'}],
    )
...
def test_sync_dimensions_exception(db):
    ...
    with mock.patch(
        'apps.quickbooks_online.utils.QBOConnector.sync_tax_codes'
    ) as mock_call:
        mock_call.side_effect = Exception()
        qbo_connection.sync_dimensions()
