...
def test_get_or_create_credit_card_or_debit_card_vendor(mocker, db):
    ...
    contact = get_or_create_credit_card_or_debit_card_vendor(
        workspace_id, '', True, general_settings
    )
    assert contact.value == 'samp_merchant'
    ...
    contact = get_or_create_credit_card_or_debit_card_vendor(
        workspace_id, 'Books by Bessie', True, general_settings
    )
...
def test_create_or_update_employee_mapping(mocker, db):
    ...
    source = ExpenseAttribute.objects.filter(
        ...
        value__iexact='user4@fyleforgotham.in',
        workspace_id=workspace_id,
    ).first()
    if source:
        employee_mapping = EmployeeMapping.objects.get(
            source_employee__value='user4@fyleforgotham.in'
        )
...
def test_changing_accounting_period(mocker, db, create_task_logs):
    ...
    with mock.patch('qbosdk.apis.bills.Bills.post') as mock_call:
        ...
        workspace_general_settings = WorkspaceGeneralSettings.objects.get(
            workspace_id=task_log.workspace_id
        )
        ...
        errors = Error.objects.filter(
            workspace_id=task_log.workspace_id,
            is_resolved=False,
            error_title='ValidationFault / 6210',
        ).all()
...
def test_post_qbo_expenses_exceptions(create_task_logs, db):
    ...
    with mock.patch(
        'apps.quickbooks_online.models.QBOExpense.create_qbo_expense'
    ) as mock_call:
        mock_call.side_effect = QBOCredential.DoesNotExist()
...
def test_post_credit_card_exceptions(mocker, create_task_logs, db):
    ...
    with mock.patch(
        'apps.quickbooks_online.models.CreditCardPurchase.create_credit_card_purchase'
    ) as mock_call:
        mock_call.side_effect = QBOCredential.DoesNotExist()
...
def test_post_journal_entry_success(mocker, create_task_logs, db):
    mocker.patch(
        'qbosdk.apis.JournalEntries.post', return_value=data['post_journal_entry']
    )
    ...
    assert (
        journal_entry.private_note
        == 'Reimbursable expense by ashwin.t@fyle.in on 2022-04-06 '
    )
...
def test_post_create_journal_entry_exceptions(create_task_logs, db):
    ...
    with mock.patch(
        'apps.quickbooks_online.models.JournalEntry.create_journal_entry'
    ) as mock_call:
        mock_call.side_effect = QBOCredential.DoesNotExist()
...
def test_post_cheque_success(mocker, create_task_logs, db):
    ...
    assert (
        cheque.private_note
        == 'Reimbursable expense by user9@fyleforgotham.in on 2020-05-13 '
    )
...
def test_post_bill_payment_exceptions(mocker, db):
    ...
    with mock.patch(
        'apps.quickbooks_online.models.BillPayment.create_bill_payment'
    ) as mock_call:
        mock_call.side_effect = BulkError(
            msg='employess not found', response='mapping error'
        )
...
def test_handle_quickbooks_errors(db):
    ...
    handle_quickbooks_error(
        exception=WrongParamsError(
            msg='Some Parameters are wrong',
            response=json.dumps({'blewblew': 'invalid_grant'}),
        ),
        expense_group=expense_group,
        task_log=task_log,
        export_type='Bill',
...
def test_schedule_reimbursements_sync(db):
    ...
    schedule = Schedule.objects.filter(
        func='apps.quickbooks_online.tasks.process_reimbursements', args=3
    ).count()
    assert schedule == 0
...
def test_process_reimbursements(db, mocker):
    ...
    mocker.patch(
        'fyle_integrations_platform_connector.apis.Reimbursements.sync', return_value=[]
    )
    workspace_id = 3
...
def test_async_sync_accounts(mocker, db):
    mocker.patch(
        'apps.quickbooks_online.utils.QBOConnector.sync_accounts', return_value=None
    )
    old_accounts = DestinationAttribute.objects.filter(
        attribute_type='ACCOUNT', workspace_id=3
    ).count()
...
def test_schedule_qbo_objects_status_sync(db):
    ...
    schedule_count = Schedule.objects.filter(
        func='apps.quickbooks_online.tasks.check_qbo_object_status', args=3
    ).count()
    assert schedule_count == 1
...
def test_schedule_credit_card_purchase_creation(db):
    ...
    credit_card_purchase = CreditCardPurchase.objects.filter(
        expense_group__id=expense_group.id
    ).first()
